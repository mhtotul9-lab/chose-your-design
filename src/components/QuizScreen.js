  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setQuiz(snap.exists() && snap.data().quiz ? snap.data().quiz : DEFAULT_QUIZ);
      } catch { setQuiz(DEFAULT_QUIZ); }
      setLoading(false);
    };
    load();
  }, []);

  const handleSelect = (i) => { if (answered) return; setSelected(i); setAnswered(true); };

  const handleNext = () => {
    const newCorrect = selected === quiz[current].ans ? correct + 1 : correct;
    if (current + 1 >= quiz.length) {
      (newCorrect / quiz.length) * 100 >= 80 ? onPass() : onFail();
    } else {
      setCorrect(newCorrect); setCurrent(c => c + 1); setSelected(null); setAnswered(false);
    }
  };
    ...
    <div style={S.page}>
      ...
      <div style={{ width: "100%", maxWidth: 520, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={S.muted}>{t.question} {current + 1} {t.of} {quiz.length}</span>
          <span style={S.badge("purple")}>{correct} {t.correct_so_far}</span>
        </div>
        <div style={{ width: "100%", height: 6, background: "#1a1730", borderRadius: 10 }}>
          <div style={{ width: `${(current / quiz.length) * 100}%`, height: "100%", background: "linear-gradient(90deg,#6c5ce7,#a78bfa)", borderRadius: 10, transition: "width 0.4s" }} />
        </div>
      </div>
      <div style={{ ...S.card, maxWidth: 520 }}>
        ...
        {answered && (
          <button style={{ ...S.btnPrimary, marginTop: 20 }} onClick={handleNext}>
            {current + 1 >= quiz.length ? t.see_results : t.next}
          </button>
        )}
      </div>
    </div>
